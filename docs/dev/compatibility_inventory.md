...
- **已移除（失败兼容）**：入口仍可识别，但只返回失败/迁移提示。
- **deprecated 保留**：只允许旧调用方过渡，禁止新功能依赖。
- **暂不处理**：本阶段不清理，但需防止扩散。
...
| ID | 兼容项 | 状态 | 主链路依赖 | 说明 |
|---|---|---|---|---|
| API-C01 | `GET /getplayerstatus` | deprecated 保留 | 否 | 已记录迁移目标 `/api/v1/player/state` |
| API-C02 | `POST /device/stop` | deprecated 保留 | 否 | 通过 facade legacy bridge 仅做过渡 |
| API-C03 | `POST /setvolume` | deprecated 保留 | 否 | 迁移目标 `/api/v1/control/volume` |
| API-C04 | `GET /playtts` | deprecated 保留 | 否 | 迁移目标 `/api/v1/control/tts` |
| API-C05 | `/cmd` 与 `/cmdstatus` | 暂不处理 | 否（不属于 v1 主链） | 属于历史控制面，单独治理 |
| API-C06 | 旧 key/code 链接鉴权 | 已移除（失败兼容） | 否 | 当前返回 410，强制迁移 |

### 3.2 Facade 层

| ID | 兼容项 | 状态 | 主链路依赖 | 说明 |
|---|---|---|---|---|
| FAC-C01 | `status(device_id)` 显式契约 | 已完成 | 是 | 已替代旧 dict 参数签名 |
| FAC-C02 | `stop_legacy(...)` | deprecated 保留 | 否 | 仅旧 router 使用 |
| FAC-C03 | `pause_legacy(...)` | deprecated 保留 | 否 | 仅旧 router 使用 |
| FAC-C04 | `tts_legacy(...)` | deprecated 保留 | 否 | 仅旧 router 使用 |
| FAC-C05 | `set_volume_legacy(...)` | deprecated 保留 | 否 | 仅旧 router 使用 |
| FAC-C06 | `PlayOptions` 契约 | 已完成 | 是 | 已替代 facade 顶层裸 dict 语义 |
| FAC-C07 | payload 键常量化 + `MediaRequest.from_payload(...)` | 已完成 | 是 | 主链路映射已集中 |

### 3.3 Source / Transport / 配置层

| ID | 兼容项 | 状态 | 主链路依赖 | 说明 |
|---|---|---|---|---|
| SRC-C01 | `LEGACY_HINT_MAP` | deprecated 保留 | 低 | 仅承接历史 hint |
| SRC-C02 | `LegacyPayloadSourcePlugin` | deprecated 保留 | 低 | compatibility only，不接新功能 |
| SRC-C03 | `DeviceRegistry._hydrate_from_legacy()` | 暂不处理 | 是 | runtime bridge，后续再抽离 |
| CFG-C01 | 旧配置字段兼容迁移 | 暂不处理 | 是 | 与播放主链路解耦，不在本轮清理 |
| MOD-C01 | 旧模块 re-export | 暂不处理 | 低 | 不阻塞 v1.1.0 Phase 1 |
