...
> [!IMPORTANT]
> `exec#...` 属于高风险能力，在 `xiaomusic-oauth2` 的安全加固版本中默认关闭。
> 本页中的 `exec#...` 示例仅适用于内网自用场景，并需要显式开启 `enable_exec_plugin=true` 且配置 `allowed_exec_commands` 白名单。
> 如果使用 `http_get/httpget` 访问外网，还需要配置 `allowlist_domains`。
