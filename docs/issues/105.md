...
从 `xiaomusic-oauth2` 的安全加固版本起，`exec#...` 属于高风险能力，默认关闭（`enable_exec_plugin=false`）。
本页内容仅建议在内网自用场景开启，并务必使用白名单限制。

启用示例（只允许执行 `code1` 与 `http_get`，且 `http_get` 只允许访问 `github.com`）：

```json
{
  "enable_exec_plugin": true,
  "allowed_exec_commands": ["code1", "http_get"],
  "allowlist_domains": ["github.com"]
}
```
...
```json
{
    ...
    "key_word_dict": {
        ...
        "测试链接": "exec#http_get(\"https://github.com/hanxi/xiaomusic\")"
    },
    "key_match_order": [
        "set_volume#",
        "get_volume#",
...
其中 "测试自定义口令" 就是对小爱音箱说的，`"exec#code1(\"hello\")"` 表示执行一个被允许的插件命令：`exec#<命令>(<字面量参数>)`。
注意：这不是“任意代码执行”，只允许“函数名 + 字面量参数”，并且必须在 `allowed_exec_commands` 白名单里。
这里 `code1` 是一个插件函数，插件函数需要在 `plugins/` 目录里实现，一个文件只会导出一个与文件名相同的插件函数，所以 `code1` 函数是在 `plugins/code1.py` 里实现的。
...
这里只是演示了打印日志和让小爱音箱说话。还有一个内置命令是 `http_get`（兼容别名 `httpget`），可以用来访问 URL（默认关闭，且需要配置 `allowlist_domains`）。
