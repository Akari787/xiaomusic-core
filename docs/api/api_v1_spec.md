...
本文档是 XiaoMusic Runtime v1 API 的唯一权威契约来源，定义所有对外 HTTP 接口行为、SSE 状态流接口行为、请求字段、响应结构与错误模型。

**改什么时必须读本文：**
- 涉及 v1 HTTP API 的任何修改（新建路由、修改接口参数或返回值）
- 涉及 SSE 状态流接口行为的任何修改
- 涉及错误码、错误模型、统一 envelope 的任何修改
- 涉及接口归属路径（Public API / Internal API / Forbidden）的划分
- 涉及接口分级（Class A / Class B / Class C）的重新定义

**AI 最容易改偏的禁止事项：**
1. 不得将 Internal API 当作 Public API 使用——未列入第 5 章白名单的接口均不属于正式契约
2. 不得将命令接口响应体中的字段用于推断播放状态——命令与状态是两条独立通道
3. 不得在 v1 契约范围内新增未声明的顶层字段或 data 字段
4. 不得修改第 15 章收敛后的统一播放入口（`POST /api/v1/play`）的行为
5. 不得将 `GET /api/v1/player/stream` 的 SSE 事件结构与投影规范定义的字段模型分离维护

**关键章节索引：**
- [第 5 章 正式白名单接口](#5-正式白名单接口)：接口归属的唯一依据
- [第 6 章 接口分级与归属路径](#6-接口分级与归属路径)：Class A/B/C 定义
- [第 7 章 成功响应契约矩阵](#7-成功响应契约矩阵)：各接口成功响应结构
- [第 8 章 统一错误模型](#8-统一错误模型)：错误码与 error_code 枚举
- [第 12 章 接口逐项契约](#12-接口逐项契约)：各接口字段级定义
- [第 15 章 统一播放入口收敛原则](#15-统一播放入口收敛原则)：播放入口禁令
...
`GET /api/v1/player/state` 的 `data` 字段必须完全符合投影规范第 5 节所定义的状态快照模型，详见第 12.24 节。该查询是纯读：不得创建/取消任务、调用 Mina 或因异常隐式修复 runtime 状态；异常只能产生安全投影并记录日志。状态权威来自 runtime phase，不得由 legacy `isplaying` 或 `_last_cmd` 覆盖。
...
约束：成功响应中的 `transport` 表示动作已进入链路，不表示播放器已到达 `playing` 状态。Coordinator deferred receipt 允许 `accepted=true`、`started=null`，表示已接受但尚未确认物理开始。调用方必须通过 `GET /api/v1/player/state` 或 SSE 流确认播放状态。
...
成功响应必须包含 `data.transport`。成功响应只承诺 stop accepted，不承诺播放器已完成物理停止；调用方须通过状态通道确认 runtime phase/`transport_state == "stopped"`。
