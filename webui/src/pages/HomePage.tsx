...
type SettingField = {
  key: string;
  label: string;
  kind?: "text" | "number" | "bool" | "password" | "textarea";
};
...
const ADVANCED_TABS: Array<{ key: string; label: string; fields: SettingField[] }> = [
  ...
  },
  {
    ...
    label: "语音TTS",
    fields: [
      { key: "edge_tts_voice", label: "Edge-TTS 语音角色" },
...
function deviceCandidates(d: Device): string[] {
  const vals = [d.did, d.deviceID, d.miotDID].filter((v): v is string => Boolean(v));
...
export function HomePage() {
  ...
  async function playCurrent() {
    await callRetApi("/playmusiclist", { did: activeDid, listname: playlist, musicname: music }, "开始播放");
  }

  async function togglePlayMode() {
    const modes = ["单曲循环", "全部循环", "随机播放", "单曲播放", "顺序播放"];
    const next = (playModeIndex + 1) % modes.length;
    setPlayModeIndex(next);
    await callRetApi("/cmd", { did: activeDid, cmd: modes[next] }, `已切换为${modes[next]}`);
  }
  ...
  async function playLink(proxy: boolean) {
    ...
    if (proxy) {
      const out = (await apiPost<{ ok?: boolean; error_code?: string; message?: string }>("/api/v1/play_url", {
        ...
        url: linkUrl,
        options: { mode: "network_audio_link", no_cache: false },
      })) as { ok?: boolean; error_code?: string; message?: string };
      setMessage(out.ok ? "代理播放已发送" : out.message || out.error_code || "代理播放失败");
      return;
    }
    const out = (await apiGet<{ ret?: string }>(
      `/playurl?did=${encodeURIComponent(activeDid)}&url=${encodeURIComponent(linkUrl)}`,
    )) as { ret?: string };
    setMessage(out.ret === "OK" ? "链接播放已发送" : out.ret || "链接播放失败");
  }
  ...
  async function confirmSearch() {
    ...
    await callRetApi("/playmusic", { did: activeDid, musicname: title, searchkey: searchKeyword }, "已发送播放");
    setShowSearch(false);
  }
  ...
  async function fetchMusicListJson() {
    const out = (await apiPost<{ ret?: string; content?: string }>("/downloadjson", {
    ...
    <>
      <div className="player" role="main" aria-label="音乐播放器">
        ...
        <label htmlFor="did" className="label-with-toggle">
          ...
          <div className="toggle-switch-container">
            ...
            <div className="toggle-switch" role="switch" aria-checked="false" aria-label="语音口令开关">
        ...
        <select id="did" className="device-selector" value={activeDid} onChange={(e) => setActiveDid(e.target.value)}>
          {devices.map((d) => {
            const did = d.did || d.deviceID || d.miotDID || "";
            return (
        ...
        <div className="buttons">
          <div className="player-controls button-group">
            <div id="modeBtn" onClick={() => void togglePlayMode()} className="control-button device-enable" role="button" tabIndex={0}>
              ...
              <span className="tooltip">切换播放模式</span>
            </div>
            ...
            <div onClick={() => void callRetApi("/device/stop", { did: activeDid }, "已停止")} className="control-button device-enable" role="button" tabIndex={0}>
              <span className="material-icons" aria-hidden="true">
                stop
              </span>
      ...
      <div className={`component ${showSettings ? "show" : ""}`} id="settings-component" style={{ display: showSettings ? "block" : "none" }}>
                ...
                settingDeviceList.map((d) => {
                  ...
                  return (
                    <label key={`did-${did}`} className="checkbox-label" style={{ display: "block" }}>
                      ...
                      />
                      {`【${d.hardware || "device"} ${did}】${d.name || d.alias || ""}`}
                    </label>
                  );
                })
              ) : (
        ...
        <div className="setting-card setting-panel">
          ...
          <div className="card-content">
            <div className="component-button-group">
              <button onClick={() => void getQrcode()}>获取二维码</button>
              <button onClick={() => void refreshOAuthRuntime()}>刷新运行时</button>
              <button onClick={() => void logoutOAuth()}>退出登录</button>
            </div>
            <p>token_valid: {String(Boolean(oauthStatus.token_valid))}</p>
            <p>runtime_auth_ready: {String(Boolean(oauthStatus.runtime_auth_ready))}</p>
            <p>login_in_progress: {String(Boolean(oauthStatus.login_in_progress))}</p>
            <p>last_error: {oauthStatus.last_error || ""}</p>
            <p>{qrcodeExpireAt ? `请使用米家 App 扫码（约 ${qrcodeRemain}s）` : qrcodeStatus}</p>
        ...
        <div className="setting-card setting-panel">
          ...
          <div className="card-content">
            ...
            <label htmlFor="active_cmd">允许唤醒的命令:</label>
            <input
              id="active_cmd"
              type="text"
              value={String(settingData.active_cmd || "")}
              onChange={(e) => updateSettingField("active_cmd", e.target.value)}
            />
