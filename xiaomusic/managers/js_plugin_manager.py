...
class JSPluginManager:
    ...
    def _start_node_process(self):
        ...
        runner_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "js_plugin_runner.js"
        )
                    ...
                    try:
                        ...
                        if line:
                            ...
                            if not stripped_line.startswith("{"):
                                self.log.debug(
                                    f"Ignoring non-JSON stdout from Node.js process: {stripped_line}"
                                )
                                continue
                        ...
                        self.log.warning(
                            f"Invalid JSON output from Node.js process, error: {e}"
                        )
                    except Exception as e:
    ...
    def _send_message(
        ...
        with self._lock:
            ...
            if not self.node_process or not self.node_process.stdin:
                raise Exception("Node.js process stdin not available")
            stdin = self.node_process.stdin
