...
def register_routers(app):
    ...
    from xiaomusic.api.routers import device, file, music, network_audio, playlist, plugin, relay, system, v1

    auth_dep = _verification_dependency()

    # Internal API 与页面入口，不属于 Public API，对外 schema 不暴露。
    app.include_router(system.router, tags=["系统管理"], include_in_schema=False)
    ...
    app.include_router(network_audio.router, tags=["网络音频(兼容)"], dependencies=[auth_dep], include_in_schema=False)
