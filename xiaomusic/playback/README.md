...
- HTTP 路由与 API 请求解析
- 前端/UI 逻辑
- relay URL 解析器、streamer、session 管理
- 单设备播放器内部定时器与播放状态细节
- 配置持久化、插件管理等无关逻辑
...
- 与 `xiaomusic/relay/`：relay 负责 relay 子系统细节，playback 不应重复实现 resolver 或 runtime。
...
- 如果代码属于 resolver、session、stream、重连策略、URL 分类，应放到 `relay/` 而不是这里。
