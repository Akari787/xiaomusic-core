...
class PlaybackFacade:
    ...
    # Legacy wrappers kept for non-v1 routes.
    async def play_url(self, url: str, speaker_id: str, options: dict[str, Any] | None = None) -> dict[str, Any]:
        out = await self.play(
            device_id=speaker_id,
            query=url,
            source_hint=(options or {}).get("source_hint", "auto"),
            options=options,
        )
        return {
            "sid": "",
            ...
            "state": "playing",
            "title": out.get("media", {}).get("title"),
            "stream_url": out.get("media", {}).get("stream_url", ""),
            "error_code": None,
            "ok": True,
            "raw": {
                "ok": True,
                "mode": "core",
                "source": out.get("source_plugin"),
    ...
    async def play_payload(self, payload: dict[str, Any], speaker_id: str) -> dict[str, Any]:
        ...
        source_hint = str(payload.get("source") or payload.get("source_hint") or "auto")
        out = await self.play(device_id=speaker_id, query=query, source_hint=source_hint)
        return {
            "sid": "",
            "speaker_id": speaker_id,
            "state": "playing",
    ...
    async def play_local_library(self, speaker_id: str, music_name: str, search_key: str = "") -> dict[str, Any]:
        ...
        return await self.play_payload(
            {"query": query, "source_hint": "local_library", "source": "local_library"},
            speaker_id,
        )
