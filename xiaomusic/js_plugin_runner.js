...
class PluginRunner {
        ...
        process.stdin.on('data', (data) => {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    logToStderr(`[JS_PLUGIN_RUNNER] Failed to parse message: ${line.trim()}`);
                    logToStderr(`[JS_PLUGIN_RUNNER] Error: ${error.message}`);
    ...
    async handleMessage(message) {
        ...
        try {
            ...
            switch (action) {
                case 'load':
                    logToStderr(`[JS_PLUGIN_RUNNER] Loading plugin: ${message.name}`);
                ...
                case 'getTopListDetail':
                    ...
                    break;
                case 'unload':
                    logToStderr(`[JS_PLUGIN_RUNNER] Unloading plugin: ${message.name}`);
            ...
            logToStderr(`[JS_PLUGIN_RUNNER] Action ${action} failed: ${error.message}`);
    ...
    loadPlugin(name, code) {
        try {
            ...
            logToStderr(`Failed to load plugin ${name}: ${error.message}`);
            throw error;
        }
    }
