...
def test_media_paths_reject_sibling_prefix_and_symlink_escape(monkeypatch, tmp_path):
    config, music_path, picture_path = _media_config(tmp_path)
    (music_path / "music_evil").mkdir()
    (music_path / "music_evil" / "secret.mp3").write_bytes(b"secret")
    ...
    assert client.get("/music/link.mp3").status_code == 404
    assert client.get("/picture/%2E%2E/music_evil/secret.mp3").status_code == 404
