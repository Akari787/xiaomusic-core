...
@pytest.mark.asyncio
async def test_init_all_data_syncs_persisted_save_time_before_fast_rebind(auth_manager):
    ...
    auth_manager.need_login = AsyncMock(return_value=False)
    auth_manager.ensure_logged_in = AsyncMock(
        side_effect=AssertionError("startup fast rebind must not login")
    )
    auth_manager.device_manager.update_device_info = AsyncMock()

    await auth_manager.init_all_data()

    debug = auth_manager.auth_debug_state()
    assert debug["login_at"] == pytest.approx(save_time_ms / 1000.0)
    assert debug["expires_at"] == 0.0
    assert debug["ttl_remaining_seconds"] == 0
    auth_manager.ensure_logged_in.assert_not_awaited()


@pytest.mark.asyncio
@pytest.mark.parametrize("save_time", [None, "not-a-timestamp"])
async def test_init_all_data_invalid_or_missing_save_time_is_zero(auth_manager, save_time):
    token_path = Path(auth_manager.auth_token_path)
    data = json.loads(token_path.read_text(encoding="utf-8"))
...
@pytest.mark.asyncio
async def test_verified_runtime_only_preserves_verified_time_without_auth_probe(auth_manager):
    auth_manager._login_at = 1234.5
    auth_manager.need_login = AsyncMock(
        side_effect=AssertionError("verified-only need_login")
    )
    auth_manager.ensure_logged_in = AsyncMock(
        side_effect=AssertionError("verified-only login")
    )
    auth_manager.device_manager.update_device_info = AsyncMock()
    ...
    assert auth_manager.auth_debug_state()["login_at"] == 1234.5
    auth_manager.need_login.assert_not_awaited()
    auth_manager.ensure_logged_in.assert_not_awaited()
