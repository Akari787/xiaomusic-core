...
@pytest.mark.unit
def test_relay_test_layers_and_runner_exist():
    assert Path("tests/unit").is_dir()
    assert Path("tests/component").is_dir()
    assert Path("tests/e2e").is_dir()
    assert Path("tools/run_relay_tests.py").is_file()


@pytest.mark.unit
def test_relay_test_runner_list_command_works():
    cmd = [sys.executable, "tools/run_relay_tests.py", "--list"]
