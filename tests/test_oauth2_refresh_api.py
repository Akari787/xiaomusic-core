...
async def test_oauth2_refresh_api_contract(monkeypatch):
    ...
    out = cast(dict[str, Any], await system.oauth2_refresh())
...
@pytest.mark.asyncio
async def test_oauth2_refresh_runtime_alias(monkeypatch):
    class _AuthManager:
        async def manual_reload_runtime(self, reason="manual_refresh_runtime"):
            return {
                "refreshed": True,
    ...
    monkeypatch.setattr(system, "xiaomusic", type("_XM", (), {"auth_manager": _AuthManager()})())
    out = cast(dict[str, Any], await system.oauth2_refresh_runtime())
    assert out["runtime_auth_ready"] is True


@pytest.mark.asyncio
async def test_auth_refresh_runtime_alias(monkeypatch):
    class _AuthManager:
        async def manual_reload_runtime(self, reason="manual_refresh_runtime"):
            return {
                "refreshed": True,
